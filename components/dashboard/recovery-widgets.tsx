export function RecoveryProgressCard({ rate }: { rate: number }) {
  ...
  return (
    <section className="rounded-[28px] border border-slate-200/80 bg-white p-6 shadow-sm">
      ...
      <p className="mt-1 text-sm text-slate-500">Ödeme Kurtarma Başarı Oranı</p>

      <div className="mt-6 flex flex-col items-center">
        <div className="relative flex size-40 items-center justify-center">
          <svg className="size-full -rotate-90" viewBox="0 0 120 120">
            <circle cx="60" cy="60" r="48" fill="none" stroke="#e5e7eb" strokeWidth="10" strokeDasharray="8 10" />
            <circle
